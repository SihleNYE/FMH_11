    ...
    <header class="site-header">
        <nav class="nav shell">
            ...
            <div class="nav-links">
                ...
                @auth
                    <span class="account-name">Hi, {{ auth()->user()->name }}</span>
                    <form method="POST" action="{{ route('logout') }}">@csrf<button class="nav-button" type="submit">Log out</button></form>
                    <a class="button button-small" href="{{ route('campaigns.create') }}">Start a campaign</a>
                @else
                    <a href="{{ route('login') }}">Log in</a>
                    <a class="button button-small" href="{{ route('register') }}">Start a campaign</a>
                @endauth
            </div>
        </nav>
    </header>
